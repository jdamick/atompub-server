    ...
    module ServiceDocument #:nodoc:
      ...
      module InstanceMethods
        ...
        def get_collections
          ...
          ActionController::Routing.possible_controllers.each do |possible_controller|
            controller = "#{possible_controller.camelize}Controller".constantize
            if controller.respond_to?(:acts_as_collection?) && !collections.include?(controller)
        ...
        def get_service_document
          ...
          collections.each do |collection|
            ...
            # add href attribute
            col.add_attribute 'href', convert_href(collection.atom_pub_opts[:href])
            # add title element
            title = REXML::Element.new 'atom:title'
            title.text = collection.atom_pub_opts[:title]
            col.add_element title
            # add accept elements
            collection.atom_pub_opts[:accept].each do |accept|
            ...
            # add categories
            if collection.atom_pub_opts[:categories]
              collection.atom_pub_opts[:categories].each do |categories|
                ...
                if categories.include?:fixed and categories[:fixed] == 'yes'
                  ...
                  cats_el.add_attribute 'href', convert_href(categories[:href])
                end
            ...
            workspaces.each do |wp|
             if REXML::XPath.first(wp, './atom:title/text()') == collection.atom_pub_opts[:workspace]
               workspace = wp
               break
             end
            end
            ...
            unless workspace
              ...
              title = REXML::Element.new 'atom:title'
              title.text = collection.atom_pub_opts[:workspace]                    
        ...
        private
          def convert_href(href)
            (href.is_a?(Proc) ? href.call(self) : href)
          end
      end
    end
  end
